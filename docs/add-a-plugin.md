...
- `assets/logo.svg` for plugin display
...
With `metadata.pluginRoot` set to `plugins`, `source` is the folder name under `plugins/`.
...
- `source` path in manifest does not match folder name.
